...
export class TeacherService {
  ...
  async findFilteredTeachers (
    ...
    return this.prismaService.teacher.findMany({where, include: {teacher_workplace: true}})
  }

  // FIND ONE Teacher BY ID
  async findOneTeacher (id: string, req: Request) {
    const teacherId = req['teacher'].id;
    const findTeacher = await this.prismaService.teacher.findFirst({where: {id}, include: {teacher_workplace: true}})
